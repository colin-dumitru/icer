var Rickshaw = {namespace: function (namespace, obj) {
...
Rickshaw.Graph = function (args) {
    ...
    this.defaults = {interpolation: "cardinal", offset: "zero", min: undefined, max: undefined};
    ...
    this.initialize = function (args) {
        ...
        var renderers = [Rickshaw.Graph.Renderer.Stack, Rickshaw.Graph.Renderer.Line, Rickshaw.Graph.Renderer.Bar, Rickshaw.Graph.Renderer.Area, Rickshaw.Graph.Renderer.ScatterPlot];
    ...
    this.configure = function (args) {
        ...
        Rickshaw.keys(this.defaults).forEach(function (k) {
            this[k] = k in args ? args[k] : k in this ? this[k] : this.defaults[k]
        }, this);
...
Rickshaw.Fixtures.Color = function () {
    ...
    this.schemes.httpStatus = {503: "#ea5029", 502: "#d23f14", 500: "#bf3613", 410: "#efacea", 409: "#e291dc", 403: "#f457e8", 408: "#e121d2", 401: "#b92dae", 405: "#f47ceb", 404: "#a82a9f", 400: "#b263c6", 301: "#6fa024", 302: "#87c32b", 307: "#a0d84c", 304: "#28b55c", 200: "#1a4f74", 206: "#27839f", 201: "#52adc9", 202: "#7c979f", 203: "#a5b8bd", 204: "#c1cdd1"};
...
Rickshaw.Graph.Ajax = Rickshaw.Class.create({initialize: function (args) {
...
Rickshaw.Graph.Axis.X = function (args) {
    ...
    this.render = function () {
        ...
        this.graph.vis.append("svg:g").attr("class", "x_grid_d3").call(axis.ticks(this.ticks).tickSubdivide(0).tickSize(gridSize));
...
Rickshaw.Graph.Axis.Y = function (args) {
    ...
    this.setSize = function (args) {
        ...
        this.vis.attr("width", this.width).attr("height", this.height * (1 + berthRate));
    ...
    this.render = function () {
        ...
        axis.tickFormat(args.tickFormat || function (y) {
            return y
        });
    ...
    this.addHighlightEvents = function (l) {
        l.element.addEventListener("mouseover", function (e) {
            ...
            self.legend.lines.forEach(function (line, index) {
                ...
                line.series.color = d3.interpolateRgb(line.series.color, d3.rgb("#d8d8d8"))(.8).toString()
            });
            self.graph.update()
        }, false);
...
Rickshaw.Graph.Behavior.Series.Order = function (args) {
    ...
    var self = this;
    $(function () {
        $(self.legend.list).sortable({containment: "parent", tolerance: "pointer", update: function (event, ui) {
            var series = [];
...
Rickshaw.Graph.Behavior.Series.Toggle = function (args) {
    ...
    if (this.legend) {
        $(this.legend.list).sortable({start: function (event, ui) {
...
Rickshaw.Graph.HoverDetail = Rickshaw.Class.create({initialize: function (args) {
    var graph = this.graph = args.graph;
    ...
    this.graph.series.active().forEach(function (series) {
        ...
        var point = {formattedXValue: xFormatter(value.x), formattedYValue: yFormatter(value.y), series: series, value: value, distance: distance, order: j, name: series.name};
    ...
    this.visible && this.render({points: points, detail: points, mouseX: eventX, mouseY: eventY, formattedXValue: formattedXValue, domainX: domainX})
}, hide: function () {
    this.visible = false;
...
Rickshaw.Graph.JSONP = Rickshaw.Class.create(Rickshaw.Graph.Ajax, {request: function () {
...
Rickshaw.Graph.RangeSlider = function (args) {
    ...
    var graph = this.graph = args.graph;
    $(function () {
        $(element).slider({range: true, min: graph.dataDomain()[0], max: graph.dataDomain()[1], values: [graph.dataDomain()[0], graph.dataDomain()[1]], slide: function (event, ui) {
...
Rickshaw.Graph.Renderer = Rickshaw.Class.create({initialize: function (args) {
    this.graph = args.graph;
    ...
    return{tension: .8, strokeWidth: 2, unstack: true, padding: {top: .01, right: 0, bottom: .01, left: 0}, stroke: false, fill: false}
}, domain: function () {
...
Rickshaw.Graph.Renderer.Line = Rickshaw.Class.create(Rickshaw.Graph.Renderer, {name: "line", defaults: function ($super) {
...
Rickshaw.Graph.Renderer.Stack = Rickshaw.Class.create(Rickshaw.Graph.Renderer, {name: "stack", defaults: function ($super) {
...
Rickshaw.Graph.Renderer.Bar = Rickshaw.Class.create(Rickshaw.Graph.Renderer, {name: "bar", defaults: function ($super) {
    ...
    var barXOffset = 0;
    var activeSeriesCount = graph.series.filter(function (s) {
        return!s.disabled
    }).length;
...
Rickshaw.Graph.Renderer.Area = Rickshaw.Class.create(Rickshaw.Graph.Renderer, {name: "area", defaults: function ($super) {
    ...
    var nodes = graph.vis.selectAll("path").data(this.graph.stackedData).enter()[method]("svg:g", "g");
...
Rickshaw.Graph.Renderer.ScatterPlot = Rickshaw.Class.create(Rickshaw.Graph.Renderer, {name: "scatterplot", defaults: function ($super) {
    return Rickshaw.extend($super(), {unstack: true, fill: true, stroke: false, padding: {top: .01, right: .01, bottom: .01, left: .01}, dotSize: 4})
}, initialize: function ($super, args) {
    $super(args)
}, render: function () {
    var graph = this.graph;
...
Rickshaw.Graph.Smoother = function (args) {
    ...
    if (this.element) {
        $(function () {
            $(self.element).slider({min: 1, max: 100, slide: function (event, ui) {
                self.setScale(ui.value);
                self.graph.update()
            }})
        })
    }
    self.graph.stackData.hooks.data.push({name: "smoother", orderPosition: 50, f: function (data) {
...
Rickshaw.Graph.Unstacker = function (args) {
    this.graph = args.graph;
    var self = this;
    this.graph.stackData.hooks.after.push({name: "unstacker", f: function (data) {
...
Rickshaw.Series = Rickshaw.Class.create(Array, {initialize: function (data, palette, options) {
    options = options || {};
    ...
    this.forEach(function (item) {
        item.data.push({x: (index * this.timeInterval || 1) + this.timeBase, y: data[item.name] || 0})
    }, this)
}, getIndex: function () {
...
Rickshaw.Series.FixedDuration = Rickshaw.Class.create(Rickshaw.Series, {initialize: function (data, palette, options) {
    var options = options || {};
